package me.kalriz.lsdn.core.secure;
...
public class Shield
{
	...
	private static final int[] KL = { 0x00000000, 0x5A827999, 0x6ED9EBA1,
			0x8F1BBCDC, 0xA953FD4E }; // Round constants for left line
	private static final int[] KR = { 0x50A28BE6, 0x5C4DD124, 0x6D703EF3,
			0x7A6D76E9, 0x00000000 }; // Round constants for right line
	...
	public static String RIPEMD160(String stringMsg)
	{
	...
	public static String RIPEMD160(byte[] msg)
	{
		// Compress whole message blocks
		Objects.requireNonNull(msg);
		int[] state = { 0x67452301, 0xEFCDAB89, 0x98BADCFE, 0x10325476,
    ...
    public static byte[] decodeBase58(String input) throws Exception {
        if (input.length() == 0) {
            return new byte[0];
        }
	...
	private static void compress(int[] state, byte[] blocks, int len)
	{
		...
		for (int i = 0; i < len; i += BLOCK_LEN)
		{
			...
			for (int j = 0; j < 80; j++)
			{
				int temp;
				temp = rotateLeft(
						al + f(j, bl, cl, dl) + schedule[RL[j]] + KL[j / 16],
						SL[j]) + el;
				al = el;
				el = dl;
				dl = rotateLeft(cl, 10);
				cl = bl;
				bl = temp;
				temp = rotateLeft(ar + f(79 - j, br, cr, dr) + schedule[RR[j]]
						+ KR[j / 16], SR[j]) + er;
				ar = er;
				er = dr;
				dr = rotateLeft(cr, 10);
